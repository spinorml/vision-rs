...
impl Yolo26Variant {
    pub fn config(&self) -> Yolo26Config {
        match self {
            Yolo26Variant::N  => Yolo26Config { depth: 0.5, width: 0.25, mc: 1024 },
            Yolo26Variant::S  => Yolo26Config { depth: 0.5, width: 0.50, mc: 1024 },
            Yolo26Variant::M  => Yolo26Config { depth: 0.5, width: 1.00, mc: 512  },
            Yolo26Variant::L  => Yolo26Config { depth: 1.0, width: 1.00, mc: 512  },
            Yolo26Variant::XL => Yolo26Config { depth: 1.0, width: 1.50, mc: 512  },
        }
    }
}
...
fn ch(base: usize, width: f32, mc: usize) -> usize {
    ((base as f32 * width).round() as usize).min(mc)
}

/// Scale a yaml repeat count by depth, minimum 1.
fn rep(base: usize, depth: f32) -> usize {
    ((base as f32 * depth).round() as usize).max(1)
}
